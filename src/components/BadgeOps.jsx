...
export default function BadgeOps({ object, styleType, setObject}) {
  const handleClickDeleteObject = (object) => {
    console.log('BadgeOps::clicked', object);
    setObject(object);
  } 
  const baseClasses =
    "flex flex-inline items-center space-x-2 text-xs font-medium me-2 px-2.5 py-0.5 rounded transition-all duration-300";
  ...
  return <div className={classes}>
            <p>{object?.name}</p> 
            <FaDeleteLeft 
              className=" hover:text-gray-800 hover:scale-105 active:scale-110 transition-all duration-200 ease-in-out cursor-pointer"
              onClick={() => handleClickDeleteObject(object)}
            />
         </div>;
}
